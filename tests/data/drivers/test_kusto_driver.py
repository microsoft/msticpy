...
_KUSTO_TESTS = [
    ("no_params", {}),
    ("cluster", {"cluster": "https://wcdscrubbedservice.kusto.windows.net"}),
...
def test_kusto_driver_queries(inst, qry_args, monkeypatch, kusto_qry_prov):
    ...
    # Run query
    result = qry_prov.MDE.scrubbeddata.list_host_processes(host_name="test", **qry_args)
...
def test_kusto_driver_params_fail(inst, qry_args, monkeypatch, kusto_qry_prov):
    ...
    if inst == "con_str":
        ...
        result = qry_prov.MDE.scrubbeddata.list_host_processes(
...
def test_kusto_driver_query_fail(inst, qry_args, monkeypatch, kusto_qry_prov):
    ...
    check.is_true(hasattr(qry_prov.MDE2.scrubbeddata, "query_new_alias"))
    ...
    if inst in ("both", "cluster", "con_str"):
        # run query
        result = qry_prov.MDE.scrubbeddata.bad_query_no_cluster(
        ...
        with pytest.raises(MsticpyParameterError):
            qry_prov.MDE.scrubbeddata.bad_query_no_cluster(cmd_line="test", **qry_args)
