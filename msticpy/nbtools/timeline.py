...
def display_timeline_dict(data: Union[pd.DataFrame, dict], **kwargs) -> figure:
    """
    Display a timeline of events.
    ...
    width: int = kwargs.pop("height", 900)
    ...
    for ser_name, series_def in data.items():
        ...
        time_col = (
            series_def["time_column"] if "time_column" in series_def else time_column
        )
...
def display_timeline_grouped(data: pd.DataFrame, **kwargs) -> figure:
    ...
    if group_by:
        ...
        colors = viridis(len(group_count_df))
        group_count_df["color"] = group_count_df.apply(
            lambda x: colors[x.y_index], axis=1
        )
...
def _create_range_tool(
    data, min_time, max_time, start_range, width, time_column: str = None
):
    """Create plot bar to act as as range selector."""
    ext_min = min_time - ((max_time - min_time) / 10)
    ext_max = max_time + ((max_time - min_time) / 10)
    rng_select = figure(
    ...
    range_tool = RangeTool(x_range=start_range)
...
def _add_ref_line(plot, ref_time, series_count=1):
    ...
    plot.line(x=[ref_label_tm, ref_label_tm], y=[0, len(series_count)])
    ref_label = Label(
        x=ref_label_tm,
        y=0,
        y_offset=10,
