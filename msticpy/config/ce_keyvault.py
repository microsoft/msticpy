...
class CEKeyVault(CESimpleBase):
    ...
    def __init__(self, mp_controls: MpConfigControls):
        ...
        for field, (f_type, f_params) in self.comp_defn.items():
            default = f_params.get("default")
            options = f_params.get("options")
            curr_value = self.settings.get(field)

            if f_type == "str":
                self.controls[field] = widgets.Text(
                    description=field, value=curr_value, **w_style
                )
            if f_type == "enum":
                self.controls[field] = widgets.Combobox(
                    description=field,
                    value=curr_value,
                    options=options,
                    style=w_style["style"],
                ...
                if curr_value is None:
                    curr_value = default.casefold() == "true"
                self.controls[field] = widgets.Checkbox(
                    description=field, value=curr_value, style=w_style["style"]
                )
