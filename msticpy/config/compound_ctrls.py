...
class ArgControl(SettingsControl, CompEditStatusMixin):
    ...
    def value(self) -> Union[str, Dict[str, Optional[str]]]:
        ...
        """
        txt_val = self.txt_val.value if self.txt_val.value != "" else None
        if self.rb_store_type.value == STORE_TEXT:
            return widget_to_py(txt_val)
        return {self.rb_store_type.value: widget_to_py(txt_val)}
...
def get_arg_ctrl(setting_path, var_name, mp_controls):
    ...
    if arg_ctrl is None:
        ...
        arg_ctrl.set_values(label=var_name, store_type=store_type, item_value=store_val)
    return arg_ctrl
    ...
    def _get_val_from_ctrl(self, val_dict):
        ...
        for name, value in val_dict.items():
            ...
            if isinstance(value, dict):
                ...
                ctrl_val[name] = widget_to_py(value.value, ctrl=value)
            else:
                ctrl_val[name] = value
        return ctrl_val
