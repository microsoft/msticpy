...
from msticpy.analysis.anomalous_sequence.utils import cmds_only, cmds_params_only, cmds_params_values, probabilities
...
class Model:
    ...
    def __init__(self, sessions: List[List[Union[str, Cmd]]]):
        ...
        for i, ses in enumerate(sessions):
            ...
            assert len(ses) > 0, 'session at index {} of sessions is empty. Each session should contain at least one ' \
                                 'command'.format(i)
    ...
    def compute_scores(self, use_start_end_tokens: bool):
        ...
        self.compute_rarest_windows(window_len=2, use_geo_mean=False, use_start_end_tokens=use_start_end_tokens)
        self.compute_rarest_windows(window_len=3, use_geo_mean=False, use_start_end_tokens=use_start_end_tokens)
    ...
    def _compute_counts(self):
        ...
        if self.session_type == SessionType.cmds_only:
            ...
            seq1_counts, seq2_counts, param_counts, cmd_param_counts = cmds_params_only.compute_counts(
                sessions=self.sessions,
                start_token=self.start_token,
                end_token=self.end_token,
                unk_token=self.unk_token
            ...
            seq1_counts, seq2_counts, param_counts, cmd_param_counts, value_counts, param_value_counts = \
    ...
    def _compute_probs(self):
        ...
        if self.session_type in [SessionType.cmds_params_only, SessionType.cmds_params_values]:
    ...
    def compute_setof_params_cond_cmd(self, use_geo_mean: bool):
        ...
        if self.session_type == SessionType.cmds_only:
            raise Exception('this method is not available for your type of input data "sessions"')
        elif self.session_type == SessionType.cmds_params_only:
    ...
    def compute_rarest_windows(self, window_len: int, use_start_end_tokens: bool = True, use_geo_mean: bool = False):
        """
        ...
        if self.session_type == SessionType.cmds_only:
            rare_tuples = [cmds_only.rarest_window_session(
                session=ses, prior_probs=self.prior_probs,
                trans_probs=self.trans_probs,
                window_len=window_len,
                use_start_end_tokens=use_start_end_tokens,
                start_token=self.start_token,
                end_token=self.end_token,
                use_geo_mean=use_geo_mean) for ses in self.sessions]
        elif self.session_type == SessionType.cmds_params_only:
            rare_tuples = [cmds_params_only.rarest_window_session(
                session=ses, prior_probs=self.prior_probs,
                trans_probs=self.trans_probs,
                param_cond_cmd_probs=self.param_cond_cmd_probs,
                window_len=window_len,
            ...
            rare_tuples = [cmds_params_values.rarest_window_session(
                session=ses, prior_probs=self.prior_probs,
                trans_probs=self.trans_probs,
                param_cond_cmd_probs=self.param_cond_cmd_probs,
                value_cond_param_probs=self.value_cond_param_probs,
    ...
    def _asses_input(self):
        ...
        if isinstance(cmd, str):
            ...
            if isinstance(cmd.params, set):
                ...
                raise Exception('params attribute of Cmd data structure should be either a set or a dict')
        else:
            raise Exception('each element of "sessions" should be a list of either strings, or Cmd data types')
