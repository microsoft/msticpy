...
def compute_counts(sessions: List[List[Cmd]], start_token: str = '##START##', end_token: str = '##END##',
    ...
    return seq1_counts, seq2_counts, param_counts, cmd_param_counts, value_counts, param_value_counts


def get_params_to_model_values(param_counts: Union[StateMatrix, dict],
                               param_value_counts: Union[StateMatrix, dict]) -> set:
    """
    ...
    """
    param_stats = [(param, len(vals), param_counts[param], 100 * len(vals) / param_counts[param])
                   for param, vals in param_value_counts.items()]

    modellable_params = [param[0] for param in param_stats if param[1] <= 20 and param[2] >= 20 and param[3] <= 10]

    return set(modellable_params)


def compute_prob_setofparams_given_cmd(cmd: str, params_with_vals: dict, param_cond_cmd_probs: Union[StateMatrix, dict],
                                       value_cond_param_probs: Union[StateMatrix, dict], modellable_params: Union[set, list],
...
def compute_likelihood_window(window: List[Cmd], prior_probs: Union[StateMatrix, dict],
                              ...
                              value_cond_param_probs: Union[StateMatrix, dict], modellable_params: set,
...
def compute_likelihood_windows_in_session(session: List[Cmd], prior_probs: Union[StateMatrix, dict],
                                          ...
                                          param_cond_cmd_probs: Union[StateMatrix, dict],
                                          value_cond_param_probs: Union[StateMatrix, dict],
                                          modellable_params: set, window_len: int,
                                          use_start_end_tokens: bool, start_token: str = None, end_token: str = None,
                                          use_geo_mean: bool = False) -> List[float]:
    """
...
def rarest_window_session(session: List[Cmd], prior_probs: Union[StateMatrix, dict],
                          trans_probs: Union[StateMatrix, dict], param_cond_cmd_probs: Union[StateMatrix, dict],
                          value_cond_param_probs: Union[StateMatrix, dict], modellable_params: set, window_len: int,
                          use_start_end_tokens: bool, start_token: str, end_token: str,
                          use_geo_mean: bool = False) -> Tuple[List[Cmd], float]:
    """
