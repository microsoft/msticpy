...
class SplunkUploader(UploaderBase):
    ...
    # Allow passing through host value
    def folder_upload(self, folder_path:str, index_name:str, table_name:str = None, delim:str = ',', ):
        if delim != ",":
            ext = "*"
        else:
            ext = "*.csv"
    ...
    def _check_index(self, index_name:str):
        service_list = [item.name for item in self.service.indexes]
    ...
    def _load_index(self, index_name):
        if self._check_index(index_name) is False:
