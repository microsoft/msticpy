...
class QueryEditor:
    ...
    def changed_data(self):
        """Return True if data has changed."""
        return self._changed_data or (self.parameters.changed_data if self.parameters else False)
...
class QueryTemplateEditor(IPyDisplayMixin):
    ...
    def __init__(self, query_collection: Union[QueryCollection, Path, str, None] = none):
        """
        Initialize the QueryTemplateEditor.
        
        Parameters
        ----------
        query_collection : Union[QueryCollection, Path, str, None], optional
        ...
        if isinstance(query_collection, (Path, str)):
...
# Load the queries from the YAML file
# query_collection = load_queries_from_yaml('c:/src/msticpy/msticpy/data/queries/mssentinel/kql_sent_az_dns.yaml')
