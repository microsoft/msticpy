...
_BASE_URL = f"https://pulsedive.com/api/"
...
class PDlookup:
    ...
    lookup_ioc(observable: str, pd_type: str) -> pd.DataFrame: Lookup an indicator of compromise in the Pulsedive API.
    explore(query: str, pd_type: str) -> pd.DataFrame: Explore different types of data available in the Pulsedive API.
    scan(observable: str, pd_type: str) -> pd.DataFrame: Submits an indicator of compromise to the Pulsedive API for scanning.
        ...
        try:
            for key, value in data.items():
                if key == "q":
                    ...
                    res = httpx.get(f'{_BASE_URL}explore.php?{key}={value}&pretty=1&key={self.pd_key}', timeout=None)
                    ...
                    if res.status_code == 200:
                        # if the initial request is successful, get the qid and check the status until it is 'success'
                        qid = res.json()['qid']
                        res = httpx.get(f"{_BASE_URL}analyze.php?qid={qid}&pretty=1&key={self.pd_key}")
                        status = res.json()['status']
                        print(status)
                        while status == 'processing':
                            result = httpx.get(f"{_BASE_URL}analyze.php?qid={qid}&pretty=1&key={self.pd_key}")
                    ...
                    res = httpx.get(f"{_BASE_URL}info.php?{key}={value}&pretty=1&key={self.pd_key}", timeout=None)
                    if res.status_code == 200:
                        return json_normalize(res.json())
                    raise ValueError(f'API request failed with status code {res.status_code} and message {res.text}')
    ...
    def lookup_ioc(self, observable: str, pd_type: str) -> pd.DataFrame:
        ...
        if pd_type not in _QUERY_OBJECTS_MAPPINGS:
            raise ValueError(f"Invalid pd_type : {pd_type}. Should be one of {_QUERY_OBJECTS_MAPPINGS.keys()}")
